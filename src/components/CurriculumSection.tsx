...
import { ibmModules } from './DataAnalyticsv2Constants';

export const CurriculumSection: React.FC = () => {
  const [openIndex, setOpenIndex] = useState<number | null>(0);

  const modules = ibmModules.map((m, idx) => {
    // Extract unique tools to use as tags
    const toolsList = m.sessions.flatMap((s: any) => s.tools ? s.tools.split(', ') : []);
    const uniqueTools = Array.from(new Set(toolsList)).slice(0, 3);
    
    // Extract topics
    const topics = m.sessions.flatMap((s: any) => s.topics);

    return {
      id: `0${idx + 1}`.slice(-2),
      title: m.title.replace(/Module \d+: /, ''),
      tags: uniqueTools.length > 0 ? uniqueTools : ["Data Analysis"],
      content: topics
    };
  });
